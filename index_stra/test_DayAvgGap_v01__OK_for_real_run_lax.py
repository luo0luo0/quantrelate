...
def OpenPosition(RecvTime, direction, open_price):
    if GlobalVar.is_hold:
        ...
        return
    print GlobalVar.last_direction
...
# 平仓
def ClosePosition(RecvTime, CloseReason, BidPrice1, AskPrice1):
    if not GlobalVar.is_hold:
        print "Error: 未持仓，无法平仓!"
        return
    ...
    thisProfit = GetProfit(GlobalVar.hold_direction, GlobalVar.open_price, close_price)
    GlobalVar.dayProfitSum += (thisProfit - HandlingFee)
    print "[%s] Close (%s) %-4s  Price: %-5.0f  本次利润: %-6.0f , 日内累积利润: %-6.0f, 策略类型: %d" \
        %(RecvTime, CloseReason, GetOppositDirection(GlobalVar.hold_direction), close_price, thisProfit, GlobalVar.dayProfitSum, GlobalVar.stratety_type)
    GlobalVar.is_hold = False
    GlobalVar.hold_direction = ""
    GlobalVar.open_price = 0
...
def ProcOneLine(line):
    ...
    ma1_pre = 0
    if len(GlobalVar.price_list[-720:-600]) > 0:
        ma1_pre = sum(GlobalVar.price_list[-720:-600]) * 1.0 / len(GlobalVar.price_list[-720:-600])
    if 0 == ma1_pre:
        ma1_pre = ma1
    #print "ma1: %f   ma1_pre: %f" %(ma1, ma1_pre)
    ...
    ''' 
    #if max(abs(GlobalVar.price_max - PreClosePrice), abs(GlobalVar.price_min - PreClosePrice)) * 1.0 / PreClosePrice  > 0.015 and \
    #   max(abs(GlobalVar.price_max - OpenPrice    ), abs(GlobalVar.price_min - OpenPrice    )) * 1.0 / OpenPrice  > 0.015 : 
    if (abs(GlobalVar.price_max - PreClosePrice) * 1.0 / PreClosePrice  > th and \
        abs(GlobalVar.price_max - OpenPrice    ) * 1.0 / OpenPrice  > th ) \
       or \
       (abs(GlobalVar.price_min - PreClosePrice) * 1.0 / PreClosePrice  > th and \
        abs(GlobalVar.price_min - OpenPrice    ) * 1.0 / OpenPrice  > th ) \
       : 
        GlobalVar.OpenBegin = True
    #print "PreClosePrice: %f  OpenPrice: %f" %(PreClosePrice, OpenPrice)
    '''
    ...
    #if (LastPrice - price_avg) * 1.0 / price_avg > 0.005:
    #    enable_open = True

    #if abs(ma1_pre - ma1) * 1.0 / ma1 <= 0.001:
    #    enable_open = False
    ...
    #if abs(LastPrice - GlobalVar.buy_sell_threadhold) > 2:  
        # 当偏离均值较远时不再开仓
    #    enable_open = False

    #print "[%s] LastPrice: %f price_avg: %f  ma1: %f" %(RecvTime, LastPrice, price_avg, ma1) 
    ...
    if iMinuteCnt > GlobalVar.test_time_interval_end - 10:
        ...
        if GlobalVar.is_hold: 
            ClosePosition(RecvTime, "收盘", BidPrice1, AskPrice1)
        return;
    ...
    if GlobalVar.enable_stop_profit and GlobalVar.is_hold and \
        ...
        ClosePosition(RecvTime, "止盈", BidPrice1, AskPrice1)
    ...
    if GlobalVar.enable_stop_loss and GlobalVar.is_hold and \
        ...
        ClosePosition(RecvTime, "止损", BidPrice1, AskPrice1)
    ...
    if 1 == GlobalVar.stratety_type:
        ...
        if ma1 > GlobalVar.buy_sell_threadhold * (1 + WideOfGap):
            ...
            modelDirection = "sell"
        elif GlobalVar.is_hold and "buy"==GlobalVar.hold_direction and ma1 < GlobalVar.buy_sell_threadhold * (1 + WideOfGap):
           modelDirection = "sell"
        elif GlobalVar.is_hold and "sell" == GlobalVar.hold_direction and ma1 > GlobalVar.buy_sell_threadhold * (1 - WideOfGap):
           modelDirection = "buy"
        else:
        ...
        if ma1 > GlobalVar.buy_sell_threadhold * (1 + WideOfGap):
           ...
           modelDirection = "ignore"
           return
    elif 3 == GlobalVar.stratety_type:
        WideOfGap = 0.0035
        if ma1 > GlobalVar.buy_sell_threadhold * (1 + WideOfGap):
            #modelDirection = "sell"
            modelDirection = "buy"
    ...
    if "buy" == modelDirection:
        if (not GlobalVar.is_hold) and enable_open and \
            ...
            print GlobalVar.is_strategy_first_trade,GlobalVar.last_direction
            OpenPosition(RecvTime, "buy", AskPrice1)
        elif GlobalVar.is_hold and 'sell' == GlobalVar.hold_direction: 
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1)
        ...
        elif GlobalVar.is_hold and 'buy' == GlobalVar.hold_direction: 
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1)

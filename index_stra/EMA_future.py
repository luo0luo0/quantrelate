...
def plot_kdj(p_list,time_list,bid_list,ask_list):
	...
	for i in range(len(p_list)):
		if i==0:
			...
			continue
		win_list = p_list[:i]
		begin_line  = float(i)/(i+2)*last_begin_line + 2.0/(i+2) * p_list[i]
		if i<win:
			last_begin_line = begin_line
			continue
		...
		begin_MA = sum(p_list[:i])/len(p_list[:i])
		boll_mid = begin_line
		...
		#if hold_price > 0 and (p_list[i] < begin_line -4  or hold_price - p_list[i] > 15):
			hold_list = p_list[hold_index:i]
			hold_win_list =[x-abs(hold_price) for x in hold_list]
			cur_gain = p_list[i]-hold_price
			if max(hold_win_list) >= 20:
				cur_gain = 20*0.5 + 0.2*5 + 0.3*cur_gain
			elif max(hold_win_list) >=5:
				cur_gain = 0.2*5 + 0.8*cur_gain
			print "Sell2cover@"+time_list[i],hold_price,p_list[i],max_hold_p,cur_gain
		...
		#if hold_price < 0 and (p_list[i]-begin_line > 4 or p_list[i]- abs(hold_price)>15) :
			hold_list = p_list[hold_index:i]
			hold_win_list =[abs(hold_price)-x for x in hold_list]
			cur_gain = abs(hold_price)-p_list[i]
			if max(hold_win_list) >= 20:
				cur_gain = 20*0.5 + 0.2*5 + 0.3*cur_gain
			elif max(hold_win_list) >=5:
				cur_gain = 0.2*5 + 0.8*cur_gain
			print "Buy2cover@"+time_list[i],hold_price,p_list[i],min_hold_p,cur_gain
			last_hold = hold_price
			gain_list.append(cur_gain)
			hold_x.append(i)
			hold_y.append(p_list[i])
...
def stra_backtest():
	...
	for fn in os.listdir(dir):
		...
		for line in open(fn):
			...
			if time >15:
				break
			if( not "2" in parts[24]) or (not "2" in parts[26]):
				continue
			p_list.append(float(parts[6]))
			time_list.append(parts[0])
			bid_list.append(float(parts[24]))
			ask_list.append(float(parts[26]))
		print len(p_list),len(time_list)
		gain_list = plot_kdj(p_list,time_list,bid_list,ask_list)
		if gain_list == []:
			continue

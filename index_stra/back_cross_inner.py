...
def OpenPosition(RecvTime, direction, open_price):
    ...
    GlobalVar.hold_max = open_price
    GlobalVar.hold_min = open_price
...
def ProcOneLine(line):
    ...
    stop_profit =30  # 止盈利润值
    if  GlobalVar.is_hold and \
        ( \
    ...
    if  GlobalVar.is_hold and \
        ( \
           ("buy" == GlobalVar.hold_direction and BidPrice1 - GlobalVar.hold_max <= -stop_loss) or \
           ('sell' == GlobalVar.hold_direction and GlobalVar.hold_min - AskPrice1 <= -stop_loss) \
        ):
        ClosePosition(RecvTime, "止损", BidPrice1, AskPrice1,1)
    if len(GlobalVar.price_list) %120!=0:
       return
    win_list = GlobalVar.price_list[-120:]
    model_direction=""
    '''
    if win_list[-1]>win_list[0]+2 and max(win_list) > win_list[-1]+3:
      model_direction = "buy"
    if win_list[-1]<win_list[0]-2 and min(win_list) < win_list[-1]-3:
      model_direction = "sell"
    '''
    if float(abs(win_list[-1]-win_list[0]))/(max(win_list)-min(win_list)) < 0.1:
       #if GlobalVar.price_list[-1] > GlobalVar.price_list[GlobalVar.last_i] +4:
       if GlobalVar.price_list[-1] > GlobalVar.price_list[-10] +2 and len(GlobalVar.price_list) - GlobalVar.last_i > 10:
           model_direction = "sell"
       #if GlobalVar.price_list[-1] < GlobalVar.price_list[GlobalVar.last_i]-4:
       if GlobalVar.price_list[-1] < GlobalVar.price_list[-10]-2 and len(GlobalVar.price_list) - GlobalVar.last_i > 10:
           model_direction = "buy"
       GlobalVar.last_i = GlobalVar.price_cnt -1
    if "buy" == model_direction:
        if (not GlobalVar.is_hold)  and \
        ...
        elif GlobalVar.is_hold and 'sell' == GlobalVar.hold_direction : #lax
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1,GlobalVar.left_ratio)
            if GlobalVar.open_price < LastPrice:
               return 1
    elif "sell" == model_direction:
        if (not GlobalVar.is_hold)  and \
        ...
        elif GlobalVar.is_hold and 'buy' == GlobalVar.hold_direction :#添加varbylax 
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1,GlobalVar.left_ratio)
            if GlobalVar.open_price > LastPrice:
               return 1
    #====================    

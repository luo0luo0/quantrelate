...
def plot_kdj(p_list,time_list):
	...
	for i in range(len(p_list)):
		...
		boll_mid = begin_line
		boll_up = boll_mid + var_win
		boll_down = boll_mid - var_win
		...
		#if hold_price > 0 and (p_list[i] < begin_line -4  or max_hold_p - p_list[i] > 20) and var_win >3:
		if hold_price > 0 and (p_list[i] < begin_line -4  or hold_price - p_list[i] > 15) and var_win >=4:
		#if hold_price > 0 and (p_list[i] < begin_line -4  or hold_price - p_list[i] > 15):
			hold_win = p_list[hold_i:i]
			hold_gain_list =[x-abs(hold_price) for x in hold_win]
			hold_gain_str = "hist:"
			for p in sorted(set(hold_gain_list)):	
				hold_gain_str += str(p)
				hold_gain_str += ":"
				hold_gain_str += str(hold_gain_list.count(p))
				hold_gain_str += "\t"
			print hold_gain_str+"\t"+str(p_list[i]-hold_price)
			cur_gain = p_list[i]-hold_price
			if max(hold_gain_list) >= 20:
				print "Sell2cover@"+time_list[i],hold_price,p_list[i],max_hold_p,0.3*(p_list[i]-hold_price )+0.5*20+0.2*5
				cur_gain = 0.3*(p_list[i]-hold_price )+0.5*20+0.2*5
			elif max(hold_gain_list)>=5:
				print "Sell2cover@"+time_list[i],hold_price,p_list[i],max_hold_p,0.8*(p_list[i]-hold_price )+0.2*5
				cur_gain =0.8*(p_list[i]-hold_price )+0.2*5
			else:
		...
		#if hold_price < 0 and (p_list[i]-begin_line > 4 or p_list[i]- min_hold_p >20) and var_win>3:
		if hold_price < 0 and (p_list[i]-begin_line > 4 or p_list[i]- abs(hold_price)>15) and var_win>=4:
		#if hold_price < 0 and (p_list[i]-begin_line > 4 or p_list[i]- abs(hold_price)>15) :
			hold_win = p_list[hold_i:i]
			hold_gain_list =[abs(hold_price)-x for x in hold_win]
			hold_gain_str = "hist:"
			for p in sorted(set(hold_gain_list)):	
				hold_gain_str += str(p)
				hold_gain_str += ":"
				hold_gain_str += str(hold_gain_list.count(p))
				hold_gain_str += "\t"
			print hold_gain_str+"\t"+str(abs(hold_price)-p_list[i])
			cur_gain = abs(hold_price)-p_list[i]
			if max(hold_gain_list) >= 20:
				print "Buy2cover@"+time_list[i],hold_price,p_list[i],max_hold_p,0.3*(abs(hold_price)-p_list[i])+0.5*20+0.2*5
				cur_gain = 0.3*(abs(hold_price)-p_list[i])+0.5*20+0.2*5
			elif max(hold_gain_list)>=5:
				print "Buy2cover@"+time_list[i],hold_price,p_list[i],max_hold_p,0.8*(abs(hold_price)-p_list[i] )+0.2*5
				cur_gain = 0.8*(abs(hold_price)-p_list[i] )+0.2*5
			else:
				print "Buy2cover@"+time_list[i],hold_price,p_list[i],max_hold_p,abs(hold_price)-p_list[i]
			last_hold = hold_price
			gain_list.append(cur_gain)
			hold_x.append(i)
			hold_y.append(p_list[i])
		...
		#if last_hold>=0 and  p_list[i] < begin_line -4   and gen_RSI(p_list[i-win:i]) < 50 and var_win > 3:
		#if   last_hold>=0 and p_list[i] < begin_line -4   and gen_RSI(p_list[i-win:i]) < 50 and var_win > 3:
		if   last_hold>=0 and p_list[i] < begin_line -4 :
			if hold_price == 0:

...
def ProcOneLine(line):
    ...
    ma1 = sum(GlobalVar.price_list[-3000:])/len(GlobalVar.price_list[-3000:])
    ma2 = sum(GlobalVar.price_list[-6000:])/len(GlobalVar.price_list[-6000:])
    model_direction=""
    '''
    ...
    '''
    if ma1 > ma2 and max(win_list) - min(win_list) < 4:
      model_direction = "buy"
    if ma2 > ma1 and max(win_list) - min(win_list) < 4:
      model_direction = "sell"

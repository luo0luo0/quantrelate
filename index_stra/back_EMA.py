...
def ProcOneLine(line):
    ...
    #如果有持仓，使用同一根线
    # 
    if GlobalVar.is_hold and "buy"==GlobalVar.hold_direction and ma1 < GlobalVar.buy_sell_threadhold + 4:
       modelDirection = "sell"
    if GlobalVar.is_hold and "sell" == GlobalVar.hold_direction and ma1 > GlobalVar.buy_sell_threadhold-4:
       modelDirection = "buy"
    if 1 == GlobalVar.stratety_type:
        WideOfGap = 0.001087
        #if ma1 > GlobalVar.buy_sell_threadhold * (1 + WideOfGap):
        if ma1 > GlobalVar.buy_sell_threadhold + 4 :#2.65
            modelDirection = "buy"
        #elif ma1 < GlobalVar.buy_sell_threadhold * (1 - WideOfGap):
        elif ma1 < GlobalVar.buy_sell_threadhold -4 :
            modelDirection = "sell"
        else:
        ...
        if ma1 > GlobalVar.buy_sell_threadhold + 4 :#6.8
            #modelDirection = "sell"
            modelDirection = "buy"
        #elif ma1 < GlobalVar.buy_sell_threadhold * (1 - WideOfGap):
        #elif ma1 < GlobalVar.buy_sell_threadhold -6:#6.8
        elif ma1 < GlobalVar.buy_sell_threadhold -4: #6.8
            #modelDirection = "buy"
            modelDirection = "sell"
        else:
            modelDirection = "ignore"
    ...
    if "buy" == modelDirection:
        ...
        elif GlobalVar.is_hold and 'sell' == GlobalVar.hold_direction and var_win > 4: #lax
        ...
        elif GlobalVar.is_hold and 'buy' == GlobalVar.hold_direction and var_win > 4:#添加varbylax 
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1,GlobalVar.left_ratio)

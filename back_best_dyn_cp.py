...
def ProcOneLine(line):
    ...
    ma1     = sum(GlobalVar.price_list[-240:]) * 1.0 / len(GlobalVar.price_list[-240:])
    ma2     = LastPrice
    #更新当日的最高价格，最低价格
    ...
    if GlobalVar.OpenBegin:
        enable_open = True
    #print len(GlobalVar.price_list),iMinuteCnt,RecvTime
    ...
    # 策略方向判断 
    #====================    
    #print ma1,GlobalVar.buy_sell_threadhold+3,GlobalVar.buy_sell_threadhold-3
    boll_thr = int( (iMinuteCnt - 9*60)/60 ) +1.5
    if GlobalVar.is_hold:
      boll_thr = max(3,boll_thr)
    if 1 == GlobalVar.stratety_type:
        WideOfGap = 0.001087
        #if ma1 > GlobalVar.buy_sell_threadhold * (1 + WideOfGap):
        if ma1 > GlobalVar.buy_sell_threadhold + boll_thr :#2.65
            modelDirection = "buy"
            #print modelDirection
        #elif ma1 < GlobalVar.buy_sell_threadhold * (1 - WideOfGap):
        elif ma1 < GlobalVar.buy_sell_threadhold -boll_thr :
            modelDirection = "sell"
            #print modelDirection
        else:
    ...
    #
    #添加震荡指标
    min_win_list = GlobalVar.price_list[-120:] 
    wave_list = [min_win_list[i+1]-min_win_list[i] for i in range(len(min_win_list)-1)]
    mean_win = sum(wave_list)/len(wave_list)
    narr = np.array(wave_list)
    narr2 = narr*narr
    sum2 = narr2.sum()
    var_wave = (sum2/len(wave_list)) - mean_win**2
    var_wave = math.sqrt(var_win)  
    
    up_wave_list= [x for x in wave_list if x>0]
    down_wave_list = [abs(x) for x in wave_list if x<0]
    RSI = sum(up_wave_list)/(sum(up_wave_list)+sum(down_wave_list))
    count_r = 1.0*len(up_wave_list)/(len(up_wave_list)+len(down_wave_list))
    if "buy" == modelDirection and ma1 - min(GlobalVar.price_list[-600:]) < 3:
        if (not GlobalVar.is_hold) and enable_open and \
            (GlobalVar.is_strategy_first_trade or (not GlobalVar.is_strategy_first_trade and "buy" != GlobalVar.last_direction)):# and ma1 - min(GlobalVar.price_list[-600:]) < 3:
            print var_win,max(GlobalVar.price_list[-600:]) - ma1,ma1 - min(GlobalVar.price_list[-600:]),ma1,GlobalVar.buy_sell_threadhold,boll_thr,count_r,RSI,var_wave
            OpenPosition(RecvTime, "buy", AskPrice1)
        elif GlobalVar.is_hold and 'sell' == GlobalVar.hold_direction  and (count_r>0.55 or count_r<0.45):# and var_win < 5: #lax
            print var_win,max(GlobalVar.price_list[-600:]) - ma1,ma1 - min(GlobalVar.price_list[-600:]),ma1,GlobalVar.buy_sell_threadhold,boll_thr,count_r,RSI,var_wave
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1,GlobalVar.left_ratio)
    elif "sell" == modelDirection and max(GlobalVar.price_list[-600:]) - ma1 < 3:
        if (not GlobalVar.is_hold) and enable_open and \
           (GlobalVar.is_strategy_first_trade or (not GlobalVar.is_strategy_first_trade and "sell" != GlobalVar.last_direction)):# and max(GlobalVar.price_list[-600:]) - ma1 < 3:
            print var_win,max(GlobalVar.price_list[-600:]) - ma1,ma1 - min(GlobalVar.price_list[-600:]),ma1,GlobalVar.buy_sell_threadhold,boll_thr,count_r,RSI,var_wave
            OpenPosition(RecvTime, "sell", BidPrice1)
        elif GlobalVar.is_hold and 'buy' == GlobalVar.hold_direction and (count_r>0.55 or count_r < 0.45):# and var_win < 5:#添加varbylax 
            print var_win,max(GlobalVar.price_list[-600:]) - ma1,ma1 - min(GlobalVar.price_list[-600:]),ma1,GlobalVar.buy_sell_threadhold,boll_thr,count_r,RSI,var_wave
            ClosePosition(RecvTime, "　　", BidPrice1, AskPrice1,GlobalVar.left_ratio)
...
def ProcOneDay(InstrumentID, datestr):
    #~/Dev/future_data/rb1701/rb1701_20161101.txt
    dataFileName = "/Users/luoaoxue/Dev/future_data/%s/%s_%s.txt" %(InstrumentID, InstrumentID, datestr)
    if not os.path.exists(dataFileName):
        print "1文件不存在: %s" %dataFileName
        return 0

...
def my_data_processor(line):
	...
	if hold_price != 0:
		...
		if hold_price > 0 and newPrice < boll_down:
		...
		if hold_price < 0 and newPrice > boll_up:
			...
			return "%s\t%s\t%d\t%s\t%s\t%s" %(checkCode1, checkCode2, modelResult, InstrumentID, mdRecvTime,str(newPrice))
	#print predict_gain
	if hold_price==0 and newPrice > begin_line + var_win :#and predict_win < thr4:
		...
		modelResult = 1
		return "%s\t%s\t%d\t%s\t%s\t%s" %(checkCode1, checkCode2, modelResult, InstrumentID, mdRecvTime,str(newPrice))
	if hold_price == 0 and newPrice < begin_line - var_win :
		print "sell",newPrice
